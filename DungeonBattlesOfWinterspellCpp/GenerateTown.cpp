...
/// </summary>
/// <returns></returns>
std::vector < std::shared_ptr<DungeonRoom>> DungeonGenerator::GeneratePlaces() {

    std::vector<std::shared_ptr<Place>> dungeonRooms;

    int roomLevel = 1;

    for (const auto& dungeon : townPlaces) {

        std::vector<std::shared_ptr<Enemy>> enemyList = GenerateEnemy(roomLevel); //npcs instead
