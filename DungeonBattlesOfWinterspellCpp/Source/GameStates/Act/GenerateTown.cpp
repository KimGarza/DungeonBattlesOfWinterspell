#include "..\..\..\Headers\GameStates\Act\GenerateTown.h"
...
///// </summary>
///// <returns></returns>
//std::vector < std::shared_ptr<Place>> GenerateTown::GeneratePlaces() {
//
//    std::vector<std::shared_ptr<Place>> places;
//
//    for (const auto& place : townPlaces) {
//
//        if (place.first == "Mages Guild") {
//            // free spell craft each level from wizard
//            // sells magic items like staves and wands. If sell them specific 2 items together she'll give you a magic item which has additional properties
//            std::shared_ptr<NPC> mage = GenerateNPC("name", NPCType::SpellCraft, wares);  
//        }
//        else if (place.first == "Armoury") {
//            std::shared_ptr<NPC> smith = GenerateNPC("name", NPCType::BlackSmith, wares); // only sells armour, takes gold or items
//        }
//        else if (place.first == "Jeweler") {
//            std::shared_ptr<NPC> jeweler = GenerateNPC("name", NPCType::Jeweler, wares); // only sells jewelry, takes gold or items but buys jewels for so much more than everyone else
//        }
//        else if (place.first == "Black Bear Tavern") {
//            std::shared_ptr<NPC> innMaster = GenerateNPC("name", NPCType::BarKeep, wares); // only sells armour, takes gold or items
//        }
//        else if (place.first == "Crystal Moon Tavern") {
//            std::shared_ptr<NPC> innMaster = GenerateNPC("name", NPCType::BarKeep, wares); // only sells armour, takes gold or items
//        }
//        else if (place.first == "Apothecary") {
//            std::shared_ptr<NPC> alchemist = GenerateNPC("name", NPCType::Alchemist, wares); // only sells potions, takes gold or items
//        }
